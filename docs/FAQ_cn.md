...
8.  **问： 在 `tools/train.py` 中，代码中是否首先在有标注数据集上训练，其目的是什么？（问题 [#26](../../../issues/23)）**

    **答：** 需要让第一次和最后一次迭代都是在有标注数据集上训练，以保证训练模型的稳定性。

9.  **问： 为什么对于未标注集，在 `epoch_based_runner.py` 的第 70-74 行也涉及到了对 GT 信息的操作（即 `gt_bboxes` 和 `gt_labels`）？如果使用完全无标注的数据作为未标注集，需要做哪些修改？（问题 [#28](../../../issues/28) 和 [#29](../../../issues/29)）**

    **答：** 这几行的操作是为了抹除未标注集图像的定位信息，这样在计算涉及到未标注集的损失时，就可以知道数据来源从而不反传梯度，如此实际上是没有使用其 GT 信息的。
    
    如果使用完全无标注的数据作为未标注集，可以在这些无标注数据的标注信息中添加任意一个定位框，该定位框的标注格式需要和其他有标注数据的标注格式保持一致。
