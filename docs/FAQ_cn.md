...
6.  **问： 当训练自定义数据集（前景类别数为 1）时，在训练过程中为何 `l_imgcls` 指标一直为 0 呢？（问题 [#23](../../../issues/23) 和 [#24](../../../issues/24)）**

    **答：** 为避免该情况，你可以在数据集中新建另一个类别，但那个类别没有对应的图像。
    
7.  **问： 在 `tools/train.py` 中，代码中是否首先在有标注数据集上训练，其目的是什么？（问题 [#26](../../../issues/23)）**

    **答：** 需要让第一次和最后一次迭代都是在有标注数据集上训练，以保证训练模型的稳定性。

8.  **问： 为什么对于未标注集，在 `epoch_based_runner.py` 的第 70-74 行也涉及到了对 GT 信息的操作（即 `gt_bboxes` 和 `gt_labels`）？（问题 [#28](../../../issues/28) 和 [#29](../../../issues/29)）**
...
2.  **问： 如果使用完全无标注的数据作为未标注集，需要做哪些修改？（问题 [#29](../../../issues/29#issuecomment-871210792)）**
